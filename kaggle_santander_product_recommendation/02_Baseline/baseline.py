...
features += ['age', 'antiguedad', 'renta', 'ind_nuevo',
...
df_lag = df.copy()
df_lag.columns = [col + '_prev' if col not in ['ncodpers',
                                               'int_date'] else col for col in df.columns]
df_lag['int_date'] += 1
...
use_dates = ['2016-01-28', '2016-02-28',
...
model = xgb.train(param, dtrn, num_boost_round=1000,
                  evals=watch_list, early_stopping_rounds=20)
...
preds_vld = preds_vld - \
    vld.as_matrix(columns=[prod + '_prev' for prod in prods])
...
best_ntree_limit = int(
    best_ntree_limit * (len(XY_trn) + len(XY_vld)) / len(XY_trn))

# %%
# XGBoost 모델 재학습!
model = xgb.train(
    param, dall, num_boost_round=best_ntree_limit, evals=watch_list)
...
preds_tst = preds_tst - \
    tst.as_matrix(columns=[prod + '_prev' for prod in prods])
