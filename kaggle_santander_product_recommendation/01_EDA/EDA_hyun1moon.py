...
#%%
# 코드 2-5. Training Data 데이터 .info() 함수로 상세보기
trn.info()
...
#%%
# 코드 2-10. 월별 금융 제품 보유 여부를 누적 막대 그래프로 시각화
...
#%%
# 코드 2-11. 누적 막대 그래프를 상대값으로 시각화하기
...
#%%
# 코드 2-12. 24개 금융 제품에 대한 "신규 구매" 데이터 생성하기
...
# labels.to_csv('../input/labels.csv', index=False) # 코드 2-13에서 labels 변수 사용

#%%
# 코드 2-13. 월별 신규 구매 누적 막대 그래프를 시각화하기
# labels = pd.read_csv('../input/labels.csv').astype(int) # 코드 2-12에서 생성한 변수 사용
# fecha_dato = trn['fecha_dato'] # AttributeError: 'Series' object has no attribute 'fecha_dato'
...
#%%
# 코드 2-14. 신규 구매 누적 막대 그래프를 상대값으로 시각화하기
label_sum_percent = (label_sum_over_time / (1.*np.asarray(label_sum_over_time).max(axis=0))) * 100
